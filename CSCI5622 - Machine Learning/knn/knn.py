...
class Knearest:
	...
	def majority(self, item_indices):
		...
		majCounter = Counter()
		majCounter.update(self._y[item_indices])		

		most_common = majCounter.most_common()
	...
	def classify(self, example):
		...
		return self.majority(self._kdtree.query([example], k=self._k, return_distance=False).flatten())
	...
	def confusion_matrix(self, test_x, test_y):
		...
		for xx, yy in zip(test_x, test_y):
			...
			classification = self.classify(xx)
			if (classification in d[yy]):
				d[yy][classification] += 1
			else:
				d[yy][classification] = 0
		return d
